...
print('Add tokens...')
new_tokens = list()
# align with origin whisper tokenizer
# new_tokens = [f"<|{i * 0.02:.2f}|>" for i in range(1501)]
with open('vocab.5525.list', 'r') as IN:
    tokens = IN.readlines()
    for token in tokens:
        token = token.strip()
...
if training_args.local_rank == 0 or training_args.local_rank == -1:
    processor.save_pretrained(os.path.join(output_dir, 'model-init'))
    model.save_pretrained(os.path.join(output_dir, 'model-init'))
